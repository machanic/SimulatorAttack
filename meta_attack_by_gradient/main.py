...
def parse_args():
    ...
    parser.add_argument("--meta_model_path", type=str,required=True, help="The path of trained meta prior model")
    ...
    parser.add_argument("--prior_arch", default="AE", type=str, choices=["AE", "VideoPred"])
    ...
    parser.add_argument("--fine_tune_interval", type=int, default=10)
...
def get_preprocess(input_channels):
    # 0~1 转换为 -2.2 到2.7之间
    if input_channels == 3:
...
def main():
    ...
    if args.prior_arch == "VideoPred":
        model_prior = MetaNetwork(VideoPredNetwork(args.dataset))
    elif args.prior_arch == "AE":
        model_prior = MetaNetwork(AutoEncoder(IN_CHANNELS[args.dataset]))
    model_prior.load_state_dict(torch.load(args.meta_model_path, map_location=lambda storage, location: storage)["state_dict"])
    preprocessor = get_preprocess(IN_CHANNELS[args.dataset])
    ...
    for batch_index, (image_id, image, label) in enumerate(loader):
        ...
        for step_index in range(args.max_query // 2):
            ...
            if (step_index + 1) % args.fine_tune_interval == 0:  # 估计一个梯度，然后用该梯度fine_tune原始网络
                ...
                prior = prior_step(prior, grad, args.prior_lr)  # 用梯度更新prior，piror最后更新到图像上
                # fine_tune
                model_prior.train()
                optimizer = SGD(model_prior.parameters(), lr=args.fine_tune_lr)
                for fine_tune_i in range(20):
                    processed_adv_image = preprocessor(adv_image).cuda()
                    out = model_prior.forward(processed_adv_image) # x, adv_target, gt_target, weight=None
                    loss = loss_mse(out, prior)
                    loss.backward()
                    optimizer.step()
                model_prior.eval()
            else:
                model_prior.eval()
                processed_adv_image = preprocessor(adv_image).cuda()
                prior = model_prior.forward(processed_adv_image)
            ...
            adv_image = proj_step(image, args.epsilon, adv_image)   # 这里貌似没有发现攻击成功就停止的代码
