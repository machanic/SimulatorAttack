...
from square_attack_RL.rl import agent
from square_attack_RL.rl.a3c_model import A3CModel
from square_attack_RL.rl.misc import async_
from square_attack_RL.rl.misc.copy_param import copy_grad, copy_param
...
class PixelWiseA3C(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def __init__(self, model, optimizer, t_max, gamma, beta=1e-2,
                 ...
                 act_deterministically=True, average_entropy_decay=0.999,
    ...
    def update(self, statevar):
        ...
        for i in reversed(range(self.t_start, self.t)):
            ...
            log_prob = self.past_action_log_prob[i]  # shape = B,1,H,W
            entropy = self.past_action_entropy[i]  #  shape = B,1,H,W
            assert log_prob.dim() == entropy.dim() == 4
    ...
    def act_and_train(self, state, reward):
        ...
        self.past_action_log_prob[self.t] = pout.mylog_prob(action)  # 根据action 选择那个action的log 概率 B,1,H,W
        self.past_action_entropy[self.t] = pout.myentropy  # B,1,H,W
