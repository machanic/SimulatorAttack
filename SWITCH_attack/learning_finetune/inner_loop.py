...
class InnerLoop(nn.Module):
    ...
    def forward_pass(self, images, gt_gradients, gt_logits, true_labels, target_labels, weights=None):
        ...
        pred_gradient = torch.autograd.grad(loss_temp, images, grad_outputs=torch.ones_like(loss_temp), create_graph=True)[0]
        loss_grad = self.mse_loss(pred_gradient, gt_gradients)
        return loss_grad + loss_knowledge_distillation

    def forward(self, task_meta_train_images, task_meta_train_gt_gradients,
        ...
        for i in range(self.num_updates):
            if i==0:
                loss = self.forward_pass(task_meta_train_images, task_meta_train_gt_gradients,task_meta_train_logits,
                                         task_meta_train_true_labels, task_meta_train_target_labels)
                grads = torch.autograd.grad(loss, self.parameters())
            else:
                loss = self.forward_pass(task_meta_train_images, task_meta_train_gt_gradients,task_meta_train_logits,
                                         task_meta_train_true_labels, task_meta_train_target_labels, fast_weights)
        ...
        loss = self.forward_pass(task_meta_test_images, task_meta_test_gt_gradients, task_meta_test_logits,
                                 task_meta_test_true_labels, task_meta_test_target_labels, fast_weights)
        loss = loss / self.meta_batch_size   # normalize loss
        grads = torch.autograd.grad(loss, self.parameters(),  allow_unused=False)
