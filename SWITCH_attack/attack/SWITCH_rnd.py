...
class SWITCH_rnd_attack(object):
    ...
    def attack_images(self, batch_index, images, true_labels, target_labels, target_model, surrogate_model, other_surrogate_models, args):
        ...
        # success_square_dict = defaultdict(list)  # image_idx in batch
...
def get_exp_dir_name(dataset, loss, norm, targeted, target_type, args):
    ...
    if args.attack_defense:
        dirname = 'SWITCH_rnd_on_defensive_model-{}-{}_loss-{}-{}'.format(dataset, loss, norm, target_str)
    else:
        dirname = 'SWITCH_rnd-{}-{}_loss-{}-{}'.format(dataset, loss, norm, target_str)
    return dirname
