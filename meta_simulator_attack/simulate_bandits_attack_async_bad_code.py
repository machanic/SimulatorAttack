...
class SimulateBanditsAttack(object):
    def __init__(self, args, meta_finetuner):
        self.dataset_loader = DataLoaderMaker.get_test_attacked_data(args.dataset, 1)  #  不断地取出一个个直到塞满一个

        self.data_queue = asyncio.Queue(maxsize=args.batch_size)
    ...
    async def data_produce(self, target_model):
        for batch_idx, data_tuple in enumerate(self.dataset_loader):
            ...
            await self.data_queue.put({"image":images, "true_label":true_labels, "position": batch_idx})
        await self.data_queue.put(None)

    def del_position_image(self, image_list, true_labels_list, prior_list, position):
        del image_list[position]
        del true_labels_list[position]
        del prior_list[position]

    # 永远得到新的图片item，组装成batch
    # 已经攻击成功，或者达到最大query次数的，从列表中删去
    # 注意adv_images是不断演化的，第一次来自于干净图
    # 所以干净图的列表顺序要和adv_images完全一致
    # q1_images_for_finetune再达到删除淘汰条件时，也要重新new一个
    # 由于替换功能的存在，每个
    async def attack(self, args, arch_name, target_model, result_dump_path):
        remain_count = self.batch_size
        image_list = []
        adv_image_list = []
        true_label_list = []
        positions = []
        ...
        while True:  # 去除for step_index in range(1, args.max_queries // 2 + 1):, 用while True, 若是发现大于max_queries的物品，就从image_list剔除出去
            remain_count = self.batch_size - len(image_list)
            for batch_index in range(remain_count):  # 攻击成功的从remain_count里去掉
                data_item = await self.data_queue.get()
                if data_item is None:
                    image = torch.zeros(1, IN_CHANNELS[args.dataset], target_model.input_size[-2], target_model.input_size[-1])
                    label = torch.zeros(1)
                    position = -1
                else:
                    image = data_item["image"]
                    label = data_item["true_label"]
                    position = data_item["position"]
                image_list.append(image.cuda())
                adv_image_list.append(image.cuda())  # 需要被替换成演化版本的
                true_label_list.append(label.cuda())
                positions.append(position)
                prior_list.append(torch.zeros(1, IN_CHANNELS[args.dataset], prior_size, prior_size).cuda())
            assert len(image_list) == self.batch_size
            images = torch.cat(image_list, 0)  # 这是原始的images, 后续的adv_images
            adv_images = torch.cat(adv_image_list, 0) # note that the element in adv_image_list must be replaced
            proj_step = proj_maker(images, args.epsilon)
            true_labels = torch.cat(true_label_list, 0)
            if args.targeted:
            ...
            filtered_positions = [p for p in positions if p >= 0]
            if len(filtered_positions) == 0:  # 全部消化完了
                break
            selected = torch.cat(positions)  # 选择这个batch的所有图片的index
            prior = torch.cat(prior_list, 1).cuda()
            dim = prior.nelement() / args.batch_size  # nelement() --> total number of elements
            exp_noise = args.exploration * torch.randn_like(prior) / (dim ** 0.5)
    ...
    def attack_all_images(self, args, arch_name, target_model, result_dump_path):
        ...
        for batch_idx, data_tuple in enumerate(self.dataset_loader):
            ...
            # use a dict to store key = batch_id , value = reference index of original data
            first_finetune = True
            q1_images_for_finetune = deque(maxlen=args.meta_seq_len)
            q2_images_for_finetune = deque(maxlen=args.meta_seq_len)
            q1_logits_for_finetune = deque(maxlen=args.meta_seq_len)
            ...
            selected = torch.arange(batch_idx * args.batch_size,
                                    min((batch_idx + 1) * args.batch_size, self.total_images))  # 选择这个batch的所有图片的index
            if args.targeted:
                if args.target_type == 'random':
                    target_labels = torch.randint(low=0, high=CLASS_NUM[args.dataset],
                                                  size=true_labels.size()).long().cuda()
            ...
            for step_index in range(1, args.max_queries // 2 + 1):
                ...
                if (step_index <= args.warm_up_steps or (
                    ...
                    q2_logits_for_finetune.append(q2_logits.detach())
                    if step_index >= args.warm_up_steps and not_done.mean().item() > use_target_threshold:
                        q1_images_seq = torch.stack(list(q1_images_for_finetune)).permute(1, 0, 2, 3,
                                                                                          4).contiguous()  # B,T,C,H,W
                        q2_images_seq = torch.stack(list(q2_images_for_finetune)).permute(1, 0, 2, 3,
                                                                                          4).contiguous()  # B,T,C,H,W
                        q1_logits_seq = torch.stack(list(q1_logits_for_finetune)).permute(1, 0,
                                                                                          2).contiguous()  # B,T,#class
                        q2_logits_seq = torch.stack(list(q2_logits_for_finetune)).permute(1, 0,
                                                                                          2).contiguous()  # B,T,#class
                        finetune_times = args.finetune_times if first_finetune else random.randint(1, 3)
                        self.meta_finetuner.finetune(q1_images_seq, q2_images_seq, q1_logits_seq, q2_logits_seq,
                                                     finetune_times,
                ...
                done_batch_id_list = np.where(not_done_np == 0)[0]
