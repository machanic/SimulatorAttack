...
class Attacker(metaclass=ABCMeta):
    ...
    @abstractmethod
    def make_adv_examples_iteration(self, images,true_labels,target_labels):
        pass

    def make_adv_examples_minibatch(self,batch_index, max_queries, images, true_labels, target_labels):
        ...
        for step_index in range(max_queries//2):
            adv_images = self.make_adv_examples_iteration(images, true_labels, target_labels)
            with torch.no_grad():
    ...
    def attack_dataset(self, args,  max_queries, result_dump_path):
        ...
        for batch_idx, (image_id,  images, true_labels) in enumerate(self.dataset_loader):
            ...
            adv_images, images, not_done_indexes = self.make_adv_examples_minibatch(batch_idx, max_queries, images, true_labels, target_labels)
            if adv_images is not None:
                ...
                all_notdone_imageid.append(image_id)
