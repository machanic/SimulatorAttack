...
class RecorderMeter(object):
  ...
  def reset(self, total_epoch):
    ...
    self.epoch_losses  = np.zeros((self.total_epoch, 2), dtype=np.float32) # [epoch, train_simulate_grad_mode/val]
    self.epoch_losses  = self.epoch_losses - 1

    self.epoch_accuracy= np.zeros((self.total_epoch, 2), dtype=np.float32) # [epoch, train_simulate_grad_mode/val]
  ...
  def plot_curve(self, save_path):
    ...
    title = 'the accuracy/loss curve of train_simulate_grad_mode/val'
    dpi = 100 
    ...
    plt.plot(x_axis, y_axis, color='g', linestyle='-', label='train_simulate_grad_mode-accuracy', lw=2)
    ...
    plt.plot(x_axis, y_axis*50, color='g', linestyle=':', label='train_simulate_grad_mode-loss-x50', lw=2)
    plt.legend(loc=4, fontsize=legend_fontsize)
