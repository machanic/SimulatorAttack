...
def convert_sparse_gradient_to_mask(shrink_coordinate_map, channels, image_height, image_width):
    """
    :param shrink_coordinate_map:  original coordinate -> shrink coordinate
    :return:
    """
    g = torch.zeros(1, channels, image_height, image_width).view(-1)
    for original_coordinate, shrink_idx in shrink_coordinate_map.items():
        g[original_coordinate] = 1
    g = g.view(1, channels, image_height, image_width)
    return g


def convert_sparse_gradients_to_mask(shrink_coordinate_map_list, channels, image_height, image_width):
    g = []
    for shrink_coordinate_map in shrink_coordinate_map_list:
        g.append(convert_sparse_gradient_to_mask(shrink_coordinate_map, channels, image_height, image_width))
