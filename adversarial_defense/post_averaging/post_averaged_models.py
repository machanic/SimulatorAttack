...
class PostAveragedNetwork(nn.Module):
    def __init__(self, model, K, R, device='cuda'):
        super(PostAveragedNetwork, model).__init__()
    ...
    def classify(self, x):
        ...
        logits, _ = padef.integratedForward(self._model, x_squad, batchSize=100, nClasses=1000, device=self._device,
...
def post_averged_model(dataset, arch):
    ...
    return PostAveragedNetwork(model, K=15, R=R)
