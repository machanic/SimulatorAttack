...
class MemoryEfficientMetaModelFinetune(object):
    def __init__(self, dataset,  batch_size, meta_arch, meta_train_type, distill_loss, data_loss, norm, targeted, use_softmax,
        ...
        # CIFAR-100@TRAIN_I_TEST_II@model_resnet34@loss_cw_l2_untargeted@epoch_3@meta_batch_size_30@num_updates_12@lr_0.001@inner_lr_0.01.pth.tar
        self.meta_model_path = "{root}/train_pytorch_model/meta_simulator_square_attack/{dataset}@{split}@model_{meta_arch}@loss_{data_loss}_{norm}_{targeted}*".format(
            root=PY_ROOT, split=SPLIT_DATA_PROTOCOL.TRAIN_I_TEST_II,
        )

        self.meta_model_path = "{root}/train_pytorch_model/meta_simulator_square_attack/{meta_train_type}@{dataset}@{split}@model_{meta_arch}@loss_{loss}@dataloss_{data_loss}_{norm}_{target_str}*".format(
