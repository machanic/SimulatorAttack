...
def pgd_attack(model, images, labels, loss_type="xent", eps=0.3, alpha=2 / 255, iters=40):
    ...
    for i in range(iters):
        ...
        gradients.append(img_grad.detach().cpu().numpy())
        eta = torch.clamp(adv_images - ori_images, min=-eps, max=eps)
        images = torch.clamp(ori_images + eta, min=0.0, max=1.0).detach_()
    return np.stack(images_over_steps), np.stack(gradients)

def generate_and_save_PGD_data(model, loss_type, arch_name, datasetname, data_loader, save_path,
                                PGD_iter_num=40, max_imgs=10000):
    count = 0
    all_images = []
    all_gradients = []
    ...
    out_path ="{}/{}/{}/{}_model_{}_images.npy".format(save_path, loss_type, datasetname, datasetname, arch_name)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    fp = np.memmap(out_path, dtype='float32', mode='w+', shape=all_images.shape)
    fp[:, :, :, :, :] = all_images[:, :, :, :, :]
    del fp

    txt_path = "{}/{}/{}/{}_model_{}_images.txt".format(save_path, loss_type, datasetname, datasetname, arch_name)
    with open(txt_path, "w") as file_obj:
        file_obj.write(str(len(all_images)))
        file_obj.flush()
    del all_images
    out_path = "{}/{}/{}/{}_model_{}_grad.npy".format(save_path,  loss_type, datasetname, datasetname, arch_name)
...
def get_already_gen_models(loss_type, save_path, datasetname):
    ...
    pattern = re.compile(".*{}_model_(.*?)_.*".format(datasetname))
    model_names = set()
...
def generate_all_models(datasetname, loss_type, PGD_iter_num, batch_size, max_imgs):

    save_path = "{}/data_PGD_without_sep_labels/".format(PY_ROOT)
    ...
    for model_name in model_names:
        ...
        data_loader = get_data_loader(datasetname, batch_size)
        model = model.cuda()
        ...
        model.eval()
        generate_and_save_PGD_data(model, loss_type, model_name, datasetname, data_loader,
                                   save_path, PGD_iter_num, max_imgs)
        del data_loader
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_imgs", type=int,default=10000)
    args = parser.parse_args()
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    os.environ['CUDA_VISIBLE_DEVICES'] = str(args.gpu)
    generate_all_models(args.dataset, args.loss_type, args.PGD_iter, args.batch_size, args.max_imgs)