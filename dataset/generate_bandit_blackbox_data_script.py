...
from config import IN_CHANNELS, IMAGE_SIZE, CLASS_NUM, IMAGE_DATA_ROOT, PY_ROOT, ALL_MODELS
...
class DatasetModelConstructor(object):
    @staticmethod
    ...
    @staticmethod
    def construct_tiny_imagenet_model(arch):
    ...
    @staticmethod
    def get_preprocessor(input_size):
    ...
    def get_data_loader(datasetname, batch_size):
        preprocessor = DatasetModelConstructor.get_preprocessor(IMAGE_SIZE[datasetname])
...
if __name__ == "__main__":
    ...
    trn_data_loader = DatasetModelConstructor.get_data_loader(args.dataset, args.batch_size)
    ...
    if args.dataset == "TinyImageNet":
        for arch, model_path in model_names.items():
            model = DatasetModelConstructor.construct_tiny_imagenet_model(arch)
        ...
        for arch, model_path in model_names.items():
            model = DatasetModelConstructor.construct_imagenet_model(arch)
            model.eval()
            models.append({"arch_name": arch, "model": model})
    else:
        for arch, model_path in model_names.items():
            model = DatasetModelConstructor.construct_cifar_model(arch, args.dataset)
            model.load_state_dict(
                torch.load(model_path, map_location=lambda storage, location: storage)["state_dict"])
            model.eval()
            model.cuda()
