...
from learning_to_attack.finetune_network import FinetuneModel
...
class SquareAttack(object):
    def __init__(self, dataset, batch_size, targeted, target_type, epsilon, norm,
                 warm_up_steps, predict_steps,
        ...
        self.finetune_network = FinetuneModel(dataset, batch_size, "resnet34", 0.01)
        self.finetune_network_margin = FinetuneModel(dataset, batch_size, "resnet34", 0.01)
    ...
    def square_attack_linf(self, model, x, y, eps, max_queries, p_init, loss_type):
        ...
        n_iters = max_queries - 1
        metrics = np.zeros([n_iters, 7])
        x_queue =  deque(maxlen=10)
        loss_queue = deque(maxlen=10)
        margin_loss_queue = deque(maxlen=10)
        img_idx_to_batch_idx = ImageIdxToOrigBatchIdx(x.size(0))
        for i_iter in range(n_iters - 1):
            ...
            if (i_iter <= self.warm_up_steps or (
                    i_iter - self.warm_up_steps) % self.predict_steps == 0):
                logits = model(torch.from_numpy(x_new).cuda().float())
                loss = self.loss(logits, torch.from_numpy(y_curr).long().cuda(),
                                 loss_type=loss_type).detach().cpu().numpy()
                margin = self.loss(logits, torch.from_numpy(y_curr).long().cuda(),
                                   loss_type='cw_loss').detach().cpu().numpy()
                x_queue.append(x_new)
                loss_queue.append(loss)
                margin_loss_queue.append(margin)
                if i_iter >= self.warm_up_steps:
                    x_list = torch.stack(list(x_queue))
                    loss_list = torch.stack(list(loss_queue))
                    margin_list = torch.stack(list(margin_loss_queue))
                    self.finetune_network.finetune(x_list, loss_list, 20, i_iter == 0, img_idx_to_batch_idx)
                    self.finetune_network_margin.finetune(x_list, margin_list, 20, i_iter == 0, img_idx_to_batch_idx)
            else:
                loss = self.finetune_network.predict(x_new, img_idx_to_batch_idx)
                margin = self.finetune_network_margin.predict(x_new, img_idx_to_batch_idx)
