...
import os
from meta_simulator_model_batch_images.meta_two_queries_distillation_learner import MetaTwoQueriesLearner
...
def parse_args():
    ...
    parser.add_argument("--inner_batch_size", type=int, default=20)
    ...
    parser.add_argument('--arch', type=str, default='efficient_densenet',help='network name')  #10 层
    parser.add_argument('--meta_learner', type=str, default="2q_distillation")
    ...
    parser.add_argument("--load_task_mode", default=LOAD_TASK_MODE.LOAD, type=LOAD_TASK_MODE, choices=list(LOAD_TASK_MODE),
                        help="load task mode")
    parser.add_argument("--study_subject", type=str, default="meta_batchimages_simulator")
...
def main():
    ...
    if not args.evaluate:
        ...
        study_subject = args.study_subject if args.data_loss_type == "xent" else "{}_{}".format(args.study_subject, "logits")
        model_path = '{}/train_pytorch_model/{}/{}@{}.pth.tar'.format(
            PY_ROOT, study_subject, args.meta_learner.upper(), param_prefix)
        os.makedirs(os.path.dirname(model_path), exist_ok=True)
        learner = MetaTwoQueriesLearner(args.dataset, args.arch, args.meta_batch_size, args.inner_batch_size, args.meta_lr, args.inner_lr,
