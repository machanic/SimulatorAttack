...
from meta_simulator_model_PriorRGF.meta_two_queries_distillation_learner import MetaTwoQueriesLearner
import argparse
...
def parse_args():
    ...
    parser.add_argument("--study_subject", type=str, default="meta_simulator_prior_RGF")
...
def main():
    ...
    if not args.evaluate:
        data_str = "{norm}_{target}".format(norm=args.adv_norm, target="untargeted_attack" if not args.targeted else "targeted_attack_{}".format(args.target_type))
        param_prefix = "{}@{}@model_{}@loss_{}@norm_{}@epoch_{}@meta_batch_size_{}@num_updates_{}@lr_{}@inner_lr_{}".format(
            args.dataset,
            args.split_protocol, args.arch, args.loss_type, data_str, args.epoch, args.meta_batch_size, args.num_updates, args.meta_lr, args.inner_lr)
        model_path = '{}/train_pytorch_model/{}/2Q_P_RGF@{}.pth.tar'.format(
        ...
        learner = MetaTwoQueriesLearner(args.dataset, args.arch, args.meta_batch_size, args.meta_lr, args.inner_lr,
                                       args.epoch, args.num_updates,
                                        args.split_protocol, args.tot_num_tasks,
                                        args.loss_type,
                                        args.adv_norm, args.targeted, args.target_type)
        resume_epoch = 0
