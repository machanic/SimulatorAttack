...
from meta_simulator_model_PriorRGF.meta_dataset.meta_two_queries_dataset import TwoQueriesMetaTaskDataset
from torch.utils.data import DataLoader
from cifar_models_myself import *
from meta_simulator_model_PriorRGF.meta_network import MetaNetwork
from meta_simulator_model_PriorRGF.inner_loop_pair_loss import InnerLoopPairLoss

from torch.optim import Adam

class MetaTwoQueriesLearner(object):
    def __init__(self, dataset, arch, meta_batch_size, meta_step_size,
                 inner_step_size,  epoch, num_inner_updates, protocol,
                 tot_num_tasks, loss_type, adv_norm, targeted, target_type):
        ...
        self.network.cuda()
        trn_dataset = TwoQueriesMetaTaskDataset(dataset, adv_norm, tot_num_tasks, protocol, targeted, target_type)
    ...
    def train(self, model_path, resume_epoch=0):
        for epoch in range(resume_epoch, self.epoch):
            for i, (q1_images, q2_images, q1_logits, q2_logits) in enumerate(self.train_loader):
                # q1_images shape = (Task_num, 2, 10, C, H, W)  q2_images shape = (Task_num, 2, 50, C, H, W)
                # q1_logits shape = (Task_num, 2, 10, #class), q2_logits shape = (Task_num, 2, 50, #class)
                itr = epoch * len(self.train_loader) + i
                # self.adjust_learning_rate(itr, self.meta_step_size, self.lr_decay_itr)
                grads = []
                support_q1_images = q1_images[:, 0, :, :, :, :]
                support_q2_images = q2_images[:, 0, :, :, :, :]
                query_q1_images = q1_images[:, 1, :, :, :, :]
                query_q2_images = q2_images[:, 1, :, :, :, :]  # (Task_num, 50, C, H, W)
                support_q1_logits = q1_logits[:, 0, :]
                support_q2_logits = q2_logits[:, 0, :]  # (Task_num, T, #class)
                query_q1_logits = q1_logits[:, 1, :]
                query_q2_logits = q2_logits[:, 1,:]  # (Task_num, T, #class)
                for task_idx in range(q1_images.size(0)):  # 每个task的teacher model不同，所以
                    task_support_q1 = support_q1_images[task_idx].cuda() # 50, C, H, W
                    task_support_q2 = support_q2_images[task_idx].cuda() # 10, C, H, W
